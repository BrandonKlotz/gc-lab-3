// Not using this code. We would use this if we used classes instead of Objects

// class CompPlayer {
//   constructor(name) {
//     this.name = 'Grant Chirpus';
//     this.health = 10;
//   }
//   attack() {
//     getNum(0, 5);
//   }
// }
//
// class User extends CompPlayer {
//   constructor(name) {
//     super(name);
//     this.name = 'height';
//     this.heath = 40;
//     this.wins = 0;
//   }
//   heal() {
//     getNum(0, 3);
//   }
// }



// Decided to use switch.

// function decidingPrompt(playerOne)  {
//   var decider = prompt('Would you like Attack, Heal, or Quit?')
//   switch (decider === 'Attack') {
//     console.log(playerOne.name + ' decides to attack.');
//     ;
//   } else if (decider === 'Heal'){ // Warning this is doing an infinite Loop... D:
//     console.log(playerOne.name + ' has healed. ' + playerOne.name + ' has ' + playerOne.health + ' health.' );
//     healUser(playerOne, playerTwo);
//   } else if(decider === 'Quit') {
//     console.log(playerOne.name + ' has Quit the game.' + playerTwo.name + ' wins.');
//     return;
//   }
// }
//
//
//
//
// function healUser(playerOne, playerTwo) {
//
// }
//
// function attack(playerOne, playerTwo) {
// }
//


// Switch

// switch (decidingPrompt) {
//   case 'Attack':
//
//     attack(playerOne, playerTwo);
//     break;
//
//   case 'attack':
//
//     attack(playerOne, playerTwo);
//     break;
//
//   case 'Heal':
//
//     magicPotion(user);
//     break;
//
//   case 'heal':
//
//     magicPotion(user);
//     break;
//
//   case 'Quit':
//     console.log(user.name + ' has quit.');
//     return;
//     break;
//
//   case 'quit':
//     console.log(user.name + ' has quit.');
//     return;
//     break;
//
//   case null:
//     console.log(user.name + ' has quit.');
//     return;
//     break;
//
//   default:
//     window.alert('Please choose to either Attack, Heal, or Quit.');
//
//     break;
//   }
// }
